# 更新日志

## 2026-06-09

### 站点

-   [静态资源](https://blog.talaxy.cn/resource) 支持上传资源 & 创建目录。

### 落页 v0.11.0

-   文档现在支持添加标签信息了。
-   现在文档支持搜索了，支持多词同时搜索。
-   问题修复：最近文档列表现在会正常展示了。

## 2025-11-12

### 站点

-   新增日志服务，可通过 [日志服务](https://blog.talaxy.cn/freesia/support/log) 查看日志。

### 落页 v0.10.0

-   现在文档支持修改工作区了。
-   严重问题修复：纠正了 docs.json 索引文件的异常时间信息。

## 2024-06-15

### 站点

-   [画廊](https://blog.talaxy.cn/gallery)：更新了 2023 年年底至 2024 年五月间的内容
-   更正了用户登录校验逻辑。

## 2023-10-25

### 站点

新增 [Python - 官方文档](https://blog.talaxy.cn/docs/python/tutorial) 笔记，同时对站点进行了稳定性更新。

### 落页 v0.9.3

-   问题修复
    -   修复了访客状态下文档页面页边留白不对称的问题。
    -   调整了「落于」的时间格式。

## 2023-10-13

### 落页 v0.9.2

提升了接口服务的稳定性。

## 2023-10-08

### 站点

-   [画廊](https://blog.talaxy.cn/gallery)：最新合集「杭州」及「温州」已上线。
-   对画廊进行了小幅度改版，优化了用户体验。
-   从旧博客网站迁移了大量笔记。

### 落页 v0.9.1

-   问题修复
    -   修复了文档页面中大纲过多时不能滚动的问题。

## 2023-09-28

### 落页 v0.9.0

重要性更新：对接口服务进行了彻底重写。

-   新特性
    -   现在支持纯文本类型文档，在创建文档时可以看到这一选项。
-   问题修复
    -   侧边栏点击某些选项时不能正常收起。

## 2023-09-20

### 落页 v0.8.0

-   新特性
    -   新增「设置」菜单，现在用户可以临时分享自己的账号。
-   优化调整
    -   面对网络错误时会有更好的文字提示。
    -   一些 UI 调整。

## 2023-09-14

### 站点

-   新特性
    -   现在用户个人信息会加密存储。

### 落页 v0.7.0

-   新特性
    -   在「开始」界面中，如果工作区过多会被折叠。
    -   全面使用了消息通知。
-   优化调整
    -   一些 UI 调整。

## 2023-09-07

### 站点

[画廊](https://blog.talaxy.cn/gallery)：最新合集「上海」已上线，七八月内容也已同步更新。

### 落页 v0.6.0

-   新特性
    -   首页增加了「文档回收站」，现在删除的文档可以在回收站查看及恢复。
    -   「最近文档」中的记录现在可以删除。
    -   现在编辑时会显示字数。
-   优化调整
    -   调整了编辑和非编辑模式下页面高度适配情况。
    -   限制了文档中图片的高度。
-   问题修复
    -   清除了站点继承的暗黑模式样式。

## 2023-08-31

### 落页 v0.5.0

-   新特性
    -   替换了与 VSCode 同款的编辑器：Monaco ！
    -   首页的工作区列表支持拖拽排序。
    -   文档增加了日期属性，用于标注文章所属日期。
-   优化调整
    -   文档处于编辑状态时退出会先询问。
    -   大量的 UI 样式调整。
    -   提高了「最近文档」功能的稳定性。
    -   工作区表单的「描述」字段支持多行输入。
-   问题修复
    -   在一些场景下，文档大纲功能会导致页面崩溃

## 2023-08-23

### 站点

-   优化调整
    -   下线了字体提取接口。

### 落页 v0.4.0

-   新特性
    -   文档现在会在右侧显示大纲。
-   优化调整
    -   开始界面现在只会显示最近 10 个文档。
    -   一些 UI 样式调整。
-   问题修复
    -   现在侧边栏能正常滚动了。

### Weaver v1.7.0

-   新特性
    -   现在任务表单的显示状态会在链接中记录。
-   问题修复
    -   若项目周期不足则不会显示「更多周期」。

## 2023-08-17

### 站点

-   优化调整
    -   改善了用户校验流程。
    -   调整了文档中代码块的样式。

### 落页 v0.3.0

-   新特性
    -   在文档页面中，可以在侧边栏中调整文档在列表中的顺序。
    -   编辑时可以用 `Ctrl+S` 直接保存。
-   优化调整
    -   文本编辑现在可以使用 tab 符。
    -   保存时会去除段落间多余的空行。
-   问题修复
    -   现在侧边栏可以正常滚动了。

## 2023-08-10

### 站点

-   新特性
    -   重新设计了用户登录逻辑，现在支持账号登出。

### 落页 v0.2.0

-   新特性
    -   现在可以删除文档了。
    -   现在会直接显示工作区或文档的可见属性。
    -   工作区文档列表增加了新建文档的按钮。
    -   现在可以修改工作区属性了。
    -   新建文档时会自动将公开属性设为与工作区一致的值。
    -   侧边栏增加了用户登录的入口。
    -   移动端现在也有侧边栏了。
-   优化调整
    -   调整了网页结构，以满足触发阅读模式的条件。
    -   文本编辑时，会强制断词折行。
    -   大量的 UI 样式调整。
-   问题修复
    -   修复了文本图片无法正常显示的问题。
    -   文本编辑时，会自动清除外部粘贴里的样式。
    -   现在更改文档属性后会自动刷新侧边栏信息。

### Weaver v1.6.0

-   新特性
    -   在任务界面里点击项目标题可以打开项目详情的表单。
    -   周期列表现在只会显示前 5 周，剩余的可以通过折叠按钮打开。
-   优化调整
    -   项目删除的按钮移动到项目详情的表单里了。

## 2023-08-03

### 落页 🍂 v0.1.0

🎉 文档工具 [「落页」](https://blog.talaxy.cn/luoye) 上线啦。

-   新特性
    -   可以创建、编辑 markdown 文档。
    -   可以创建工作区来管理你的文档分类。
    -   可以将你的文档设置为公开，分享给其他人。

### Weaver v1.5.1

-   优化调整
    -   在任务视图中，如果选中的周期为正在进行中的周期，网页链接会去除周期的 `id` 。
    -   对于移动端，去除了会造成干扰的 hover 样式。

## 2023-07-27

### 站点

-   优化调整
    -   对于文档页面，当存在左边栏时，增加了正文的左边距。
    -   静态资源管理页面增加了登录按钮。

### WordBank 1.3

-   优化调整
    -   点击「编辑」后会自动增加一栏空行。

### Weaver 1.5

-   新特性
    -   当前进行中的周期会有特殊标记。
-   问题修复
    -   修复了任何情况下保存任务都会更改「完成时间」的问题。

## 2023-07-20

### 站点

-   新特性
    -   静态资源接入用户校验。
    -   导航栏暂时下线「Overwatch」入口，同时右上角会显示「版本更新」入口。
-   优化调整
    -   调整了页脚的样式。
-   问题修复
    -   注册 token 现在会正常保留。

### Weaver 1.4

-   新特性
    -   现在「已完成」的任务会显示「完成时间」。
-   优化调整
    -   针对移动端的任务表单样式做了调整，不再拥挤了。
    -   限定了用户光标选择的区域。
    -   统一了表单样式，包括按钮、输入框、选择框等。
    -   调整了项目标题及「新建周期」的位置。
    -   项目卡片及任务表单增加了入场动画。
    -   当前日期若匹配不上任何周期，则默认显示任务池。

## 2023-07-14

### 站点

-   新特性
    -   文档支持 mermaid 语法。

### WordBank 1.2

-   新特性
    -   使用新的 token 用户校验。
    -   现在会记录单词书的修改时间。
-   优化调整
    -   查词现在会短暂记录查询历史，减少重复查询。
    -   编辑时切换书籍会先询问。
    -   移动端顶部的单词书 Tab 间间距进行了调整。
    -   词数显示现在置于标题下面。
    -   单词书的创建时间显示去零。
-   问题修复
    -   现在创建或删除单词书后会正确刷新页面。

### Weaver 1.3

-   新特性
    -   现在项目里新增「任务池」分栏，用于暂存任务。
    -   现在创建项目时可以设置第一周的开始时间。
    -   现在任务卡片会有滑入动画。
    -   现在会记录任务的修改时间。
-   优化调整
    -   任务进度支持更细的档位。
    -   移动端现在也会展示周期列表。
    -   修改任务时，若任务没有进度会自动增加默认进度。
    -   现在项目卡片的样式与项目周期一致。
    -   现在任务卡片宽度会根据窗口宽度自动调整。
-   问题修复
    -   取消了整屏滚动的限制，使得移动端浏览器可以正常滚动刷新。
    -   修复了左侧栏的高度问题。

## 2023-07-08

### 站点

-   新特性
    -   现在将使用 token 进行用户校验，可以在 [权限设置](https://blog.talaxy.cn/user) 页进行登录。
-   优化调整
    -   去除了首页「站点迁移」的提示。

### WordBank 1.1

-   新特性
    -   增加了整词翻译的按钮。
    -   现在词表底部会显示词数。
-   优化调整
    -   现在单词也会根据 `/` 分词了。
    -   填写新词时会自动将页面滚动到底部。
    -   网页链接去除了 id 裁切的逻辑，即现在网页链接中的 id 会完整显示。
-   问题修复
    -   修复了 Safari 中，编辑单词书时清空输入会自动跳到下一个输入框的问题。

### Weaver 1.2

-   优化调整
    -   任务列表现在会根据优先级排列。
    -   更改了任务表单中的状态选项的按钮的样式，使更符合观感。
    -   当任务置为「测试中」或「已完成」时进度会自动设为 100%。
    -   「已完成」栏的任务现在会置灰。
    -   左侧栏宽度做了点调整，以适配 Safari 。
-   问题修复
    -   确定了创建项目的输入框表单类型
    -   修复了项目卡片中「创建时间」在暗黑模式下的颜色。
    -   修复了添加任务的按钮会一直置于窗口底部的问题。

## 2023-06-30

### Weaver 1.1

-   新特性
    -   任务支持进度展示及设置。
    -   使用了第二版用户校验。
    -   任务卡片支持拖动。
-   优化调整
    -   项目新建周期前会先询问。
    -   支持暗黑模式。
    -   任务表单中「状态」调整到「标题」下方，「提交」按钮改为「保存」。
    -   任务列表间增加了点间距。

## 2023-06-23

### 站点

站点现在支持第二版用户校验。在第二版中，用户需要一个 `id` 和 `API key` 作为信息登记。用户在登记前需要向管理员提出申请，管理员会提供一个 token 用于注册。

-   新特性
    -   加入了第二版用户校验，包括「权限设置」、「Token 生成」、「登记用户」三个页面。

### Weaver 1.0

Weaver 迭代工具正式发布

-   新特性
    -   任务现在会记录创建时间。
    -   项目和任务现在可以被删除。
-   优化调整
    -   新建任务时关闭状态选项。
    -   任务卡片会展示优先级。
    -   任务列表滚动条显示优化。
    -   任务表单提交按钮样式调整。
    -   任务表单的表单标题调整。
