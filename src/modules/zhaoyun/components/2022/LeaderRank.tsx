...
import { HeroName, HERO_DATA } from '../../../../constants/zhaoyun/Hero';
...
const LeaderRank = () => {
    ...
    return (
        <Container>
            {TEAM_LEADER_RANK.slice(0, fold ? 5 : TEAM_LEADER_RANK.length).map(([player, data]) => {
                const hero = HeroName.TBD;
                ...
                return (
                    ...
                    >
                        ...
                        <Spacer />
                        <div>{showNum ? `${data.win} 胜 ${data.loss} 负` : rate.toFixed(0) + '%'}</div>
                    </Row>
                );
            })}
            <div className="button-bar">
                ...
                <Spacer />
                <a onClick={() => setShowNum((prev) => !prev)}>{showNum ? '显示胜率' : '显示胜负'}</a>
            </div>
        </Container>
    );
};
